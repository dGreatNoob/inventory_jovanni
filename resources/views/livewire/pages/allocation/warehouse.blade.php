...
<div>
        ...
        @if($showStepper)
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg mb-6">
                ...
                <div class="p-6">
                    ...
                    @if($currentStep === 1)
                        <div>
                            <h4 class="text-md font-medium mb-4">Step 1: Create New Batch</h4>
                    ...
                    @if($currentStep === 4)
                        <div>
                            <h4 class="text-md font-medium mb-4">Step 4: Dispatch Batch</h4>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
                            ...
                            @if($currentBatch)
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
                                    <h5 class="font-medium mb-3">Batch Summary</h5>
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <div class="font-medium">{{ count($productAllocations) }}</div>
                                        </div>
                                    </div>
                                </div>
    ...
    <!-- STEPPER WORKFLOW MODAL -->
    <x-modal wire:model="showCreateBatchModal" class="max-w-4xl">
        <div class="mb-6">
            <h2 class="text-xl font-bold mb-2">Create New Batch Allocation</h2>
            <p class="text-sm text-gray-600 dark:text-gray-400">
                Step-by-step process for creating and managing batch allocations
            </p>
        </div>
        ...
        <div class="min-h-[400px]">
            ...
            @if($currentStep === 3)
                <div>
                    ...
                    <!-- Add Product Form -->
                    @if(isset($selectedBranchAllocationId) && $selectedBranchAllocationId)
                        @php
                            $selectedBranchAllocation = $currentBatch ? $currentBatch->branchAllocations->find($selectedBranchAllocationId) : null;
                        @endphp
                        
                        @if($selectedBranchAllocation)
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-4">
                                <h4 class="font-medium mb-2">Adding products to: {{ $selectedBranchAllocation->branch->name }}</h4>
                                
                                <!-- Existing Items -->
                                @if($selectedBranchAllocation->items->count() > 0)
                                    <div class="mb-4">
                                        <h5 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Current Items:</h5>
                                        @foreach($selectedBranchAllocation->items as $item)
