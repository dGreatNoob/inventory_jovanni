...
<div class="pb-6">
    @if(!$this->motherDr)
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-8 text-center">
            <p class="text-gray-600 dark:text-gray-400 mb-4">Summary DR not found or invalid.</p>
            <a href="{{ route('allocation.for-dispatch') }}" class="inline-flex items-center text-blue-600 dark:text-blue-400 font-medium hover:underline" wire:navigate>Back to For Dispatch DRs</a>
        </div>
        @return
    @endif
    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden mb-6">
        <div class="flex flex-wrap items-start justify-between gap-4 px-6 py-4">
            <div>
                <h1 class="text-xl font-semibold text-gray-900 dark:text-white mb-3">Summary DR: <span class="font-mono">{{ $mother->dr_number }}</span></h1>
            ...
            <div class="flex flex-wrap gap-2">
                @if($branchAllocationId)
                    <a href="{{ route('allocation.scan') }}?branch_allocation_id={{ $branchAllocationId }}"
                        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 bg-white dark:bg-gray-700 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 focus:ring-blue-500 focus:border-blue-500"
                        wire:navigate>
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 min-w-full">
                        <thead class="text-sm text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Qty</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($row->items as $item)
                                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    ...
                                    <td class="px-6 py-4 text-sm text-right font-medium text-gray-900 dark:text-white">{{ $item->scanned_quantity }}</td>
                                </tr>
                            @empty
                                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800">
                                    <td colspan="3" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No items scanned in this box.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
