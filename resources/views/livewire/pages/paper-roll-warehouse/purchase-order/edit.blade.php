        ...
        <x-collapsible-card title="Purchase Order Details" open="true" size="full">
            <form wire:submit.prevent="submit">
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <x-dropdown wire:model="del_to" name="del_to" label="Receiving Department" :options="$departments->pluck('name', 'id')->toArray()" placeholder="Select department" searchable="true" />
                    ...
                    <x-input type="text" wire:model="approver" name="approver" label="Approver" />
                                ...
                                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                            ...
                                            <div class="mt-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg dark:border-gray-600">
                                                @forelse($rawMatProfiles as $rawmat)
                                                    <div wire:click="selectRawMatProfile({{ $rawmat->id }})" class="p-3 hover:bg-blue-50 dark:hover:bg-blue-900/30 cursor-pointer {{ $selected_rawmat_profile == $rawmat->id ? 'bg-blue-100 dark:bg-blue-900/50 border-l-4 border-blue-500' : '' }}">
                                                        <div class="font-medium text-gray-900 dark:text-white">{{ $rawmat->classification }}</div>
                                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ $rawmat->gsm }} - {{ $rawmat->width_size }}</div>
                                                    </div>
                                                @empty
                            ...
                            <tbody>
                                @forelse($paginatedOrderedItems as $index => $item)
                                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">{{ number_format($item['width_size'], 2) }}</td>
