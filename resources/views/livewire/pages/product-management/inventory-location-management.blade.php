<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Inventory Locations</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage your inventory storage locations</p>
        </div>
        <div class="mt-4 sm:mt-0">
            <flux:modal.trigger name="create-edit-location">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900 dark:text-white">{{ $stats['total'] ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg">
            <div class="p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Active Locations</dt>
                            <dd class="text-lg font-medium text-gray-900 dark:text-white">{{ $stats['active'] ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900 dark:text-white">{{ $stats['warehouses'] ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg">
            <div class="p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Stores</dt>
                            <dd class="text-lg font-medium text-gray-900 dark:text-white">{{ $stats['stores'] ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    @if(!empty($selectedLocations))
                        <button wire:click="openBulkActionModal" 
                                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 dark:bg-gray-500 dark:hover:bg-gray-400">
                            Bulk Actions ({{ count($selectedLocations) }})
                        </button>
                    @endif
                </div>
            </div>
        ...
        <div class="px-4 py-5 sm:p-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-200 uppercase tracking-wider">
                                    <button wire:click="sortBy('type')" class="group inline-flex">
                                        Type
                                        @if($sortBy === 'type')
                                            @if($sortDirection === 'asc')
                                                <svg class="ml-2 h-4 w-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-200 uppercase tracking-wider">Address</th>
                            ...
                            @foreach($locations as $location)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <input type="checkbox" 
                                               wire:click="toggleLocationSelection({{ $location->id }})"
                                               @if(in_array($location->id, $selectedLocations)) checked @endif
                                               class="h-4 w-4 text-gray-600 focus:ring-gray-500 dark:focus:ring-gray-400 border-gray-300 rounded">
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $location->name }}</div>
                                        @if($location->description)
                                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ Str::limit($location->description, 50) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            @if($location->type === 'warehouse') bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300
                                            @elseif($location->type === 'store') bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                                            @elseif($location->type === 'office') bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-300
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            @if($location->is_active) bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No locations found</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by creating a new inventory location.</p>
                    <div class="mt-6">
    ...
    <!-- Modals -->
    @include('livewire.pages.product-management.modals.create-edit-location')
    @include('livewire.pages.product-management.modals.delete-location')
    @include('livewire.pages.product-management.modals.bulk-actions-location')
</div>
